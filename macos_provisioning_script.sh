...
#NEW: Determines if Signed System Volume is enabled or not (returns string). Compatible with macOS 12 only
...
if ([ $VERSION_MAJOR == "10" ] && [ $VERSION_MINOR -ge "15" ]) || [ $VERSION_MAJOR -ge "12" ]; then
...
if ([ $VERSION_MAJOR == "10" ] && [ $VERSION_MINOR -ge "14" ]) || [ $VERSION_MAJOR -ge "12" ]; then
